#ifndef ELEMENT_IS_SINK_H
#define ELEMENT_IS_SINK_H
#include "GameRule.h"
/**
 * @brief The Element_Is_Sink class is a concrete implementation of the GameRule class that represents the "sink" rule for certain elements in the game.
 */
class Element_Is_Sink :public GameRule {

public:
    /**
     * @brief This method applies the "sink" rule to an element and updates the board loader accordingly.
    ...
    inline void apply(Element elem, BoardLoader* boardLoader) override {
        ...
        std::vector<Element> pushableElems{};

        for (auto &push : boardLoader->getRockPtr()){
            pushableElems.push_back(push);
        }
        ...
        for (auto tem : temp){
            for (auto cur : currentElem){
                if (cur.getPosition() == tem.getPosition()){
                    ...
                    boardLoader->getBoardPtr()->getSquares()[cur.getPosition().getRow()][cur.getPosition().getColumn()]->removeElement(&tem);
                    deadElem.push_back(cur);
                    elemDead = true;
                }
            }
            for (auto push : pushableElems){
                if (push.getPosition() == tem.getPosition()){
                    boardLoader->getBoardPtr()->getSquares()[push.getPosition().getRow()][push.getPosition().getColumn()]->removeElement(&push);
                    boardLoader->getBoardPtr()->getSquares()[push.getPosition().getRow()][push.getPosition().getColumn()]->removeElement(&tem);
                }
            }
        }
    ...
    /**
     * @brief This method  disapply the "sink" rule.
