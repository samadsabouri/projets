...
package g53960.atl.anagram.client.client;
 ...
 * <ul>
 * <li> Method <code>sendToServer()</code> is not declared final anymore. This
 * allows user of the framework to override it, perhaps to perform some
 * filtering before sending the message to the server. However, any overriden
 * version of this method should include a call to the original one.
 * <li> A test is made before calling the <code>handleMessageFromServer</code>
 ...
 * <ul>
 * <li> The <code>run()</code> method now calls the
 * <code>connectionException</code> callback when an object of unknown class is
 * received from the input stream or when the message handler throw a
 * <code>RuntimeException</code>.
 * <li> The <code>connectionClosed</code> callback might be called after
 * <code>connectionException</code> if the exception causes the end of te
 * thread.
 ...
 * <code>closeConnection</code> to <code>run</code> method to garantee that
 * connection is really closed when this callback is called.
 * </ul><p>
 *
...
// INSTANCE METHODS *************************************************
    ...
    final public void openConnection() throws IOException {
        ...
        } catch (IOException ex) // All three of the above must be closed when there is a failure
