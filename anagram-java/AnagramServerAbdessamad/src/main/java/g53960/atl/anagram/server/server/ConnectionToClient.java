...
package g53960.atl.anagram.server.server;
 ...
 * <ul>
 * <li> A new hook method called <code>handleMessageFromClient()</code> has been
 * added. It allows to handle messages inside the client thread when possible.
 * <li> Constructor is now protected.
 * <li> Method <code>sendToClient()</code> is not declared final anymore. This
 * allows user of the framework to override it, perhaps to perform some
 * filtering before sending the message to the client. However, any overriden
 * version of this method should include a call to the original one.
 * <li> A test is made before calling the <code>handleMessageFromClient</code>
 * method such that when <code>closeConnection</code> returns, it is garanteed
 * that no new messages will be handled.
 * </ul>
 ...
 * server callback when an object of unknown class is received from the input
 * stream or when the message handler throw a <code>RuntimeException</code>
 ...
 * <code>close</code> to <code>run</code> method to garantee that connection is
 * really closed when this callback is called.
 * </ul><p>
 *
...
//            AbstractServer server) throws IOException {
    protected ConnectionToClient(List<Thread> group, Socket clientSocket,
            AbstractServer server) throws IOException {
        group.add(this);
